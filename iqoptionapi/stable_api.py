...
class IQ_Option:
    ...
    def __get_binary_open(self) -> None:
        """
        Atualiza o status de abertura de pares binários e turbo com base nas informações de `get_all_init_v2`.
        Protege contra acessos inválidos e ausência de dados.
        """
        binary_data = self.get_all_init_v2()
        binary_list = ["binary", "turbo"]

        if not binary_data:
            print("[ERRO] Dados binários/turbo indisponíveis.")
            return

        for option in binary_list:
            option_data = binary_data.get(option)
            if not option_data:
                continue

            actives = option_data.get("actives", {})
            for active_id, active in actives.items():
                name_raw = active.get("name", "")
                if "." not in name_raw:
                    continue  # ignora nomes inesperados

                name = name_raw.split(".")[1]
                enabled = active.get("enabled", False)
                is_suspended = active.get("is_suspended", True)

                # Garante que a estrutura exista
                if name not in self.OPEN_TIME[option]:
                    self.OPEN_TIME[option][name] = {}

                if enabled:
                    self.OPEN_TIME[option][name]["open"] = not is_suspended
                else:
                    self.OPEN_TIME[option][name]["open"] = False
 
    def __get_digital_open(self) -> None:
        """
        Atualiza o status de abertura de opções digitais com base no horário atual.
        Protege contra ausência de dados retornados pela API.
        """
        digital_data_raw = self.get_digital_underlying_list_data()
        digital_data = digital_data_raw.get("underlying") if digital_data_raw else None

        if not digital_data:
            print("[ERRO] Dados digitais indisponíveis ou malformados.")
            return
        ...
        for digital in digital_data:
            ...
            # Garante que a chave do ativo exista no dicionário OPEN_TIME
            if name not in self.OPEN_TIME["digital"]:
                self.OPEN_TIME["digital"][name] = {}
            ...
            for schedule_time in schedule:
                ...
                if start is not None and end is not None and start < current_time < end:
                    ...
                    break  # Já está aberto, não precisa checar os demais horários
